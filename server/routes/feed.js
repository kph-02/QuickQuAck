...
router.get("/home-feed", authorization, async (req, res) => {
  ...
  router.get("/filtered-feed", authorization, async (req, res) => {
    try {
      ...
      let sql = 'SELECT p.post_id, p.user_id, p.post_text, p.time_posted, pt.tag_id FROM post AS p JOIN post_tags as pt ON pt.post_id = p.post_id JOIN tags AS t on t.tag_id = pt.tag_id WHERE (t.tag_id = ${tag}) ORDER BY time_posted ASC;'
...
router.get("/filtered-feed2", authorization, async (req, res) => {
    try {
      ...
      let sql = 'SELECT p.post_id, p.user_id, p.post_text, p.time_posted, pt.tag_id FROM post AS p JOIN post_tags as pt ON pt.post_id = p.post_id JOIN tags AS t on t.tag_id = pt.tag_id WHERE (t.tag_id = ${tag} OR t.tag_id = ${tag2}) ORDER BY time_posted ASC;'
